...
def main():
    ...
    '--select',
    type=int,
    help='Specify the index of the result to select (1-based index). If not provided, the user will be prompted interactively.')
            ...
            if len(args.arguments) > 3:
                ...
                if modulename in orpheus.module_list:
                    ...
                    if lucky_mode:
                        ...
                        selection = args.select - 1
                        if selection < 0 or selection >= len(items):
                            raise Exception(f'Invalid selection index: {args.select}. Must be between 1 and {len(items)}.')
                    else:
                        # Imprimir los resultados en el formato solicitado
                        for index, item in enumerate(items, start=1):
                            album = item.name if item.name else 'Unknown Album'
                            artists = ', '.join(item.artists) if isinstance(item.artists, list) else (item.artists or 'Unknown Artist')
                            year = item.year if item.year else 'Unknown Year'
                            result_id = item.result_id if item.result_id else 'Unknown ID'
                            print(f'{index}. {album}|{artists}|{year}|{result_id}')
                
                        # No retornamos aquí, dejamos que el flujo continúe para luckysearch
                        if orpheus_mode == 'search':
                            return  # Termina la ejecución solo para "search"

                    # Seleccionamos el ítem para luckysearch o según el índice proporcionado
